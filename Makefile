...
_builtin_clean_generated \
	_builtin_release_managed-git-tool \
...
# Operator contract: setup provisions the locked virtual environment.
...
_builtin_release_managed-git-tool: _builtin_require_environment
	@manifest="$(strip $(MANIFEST))"; \
	if [ -z "$$manifest" ] || [ ! -f "$$manifest" ]; then \
		printf 'ERROR: MANIFEST must name one existing managed Git tool JSON file\n' >&2; \
		exit 2; \
	fi; \
	$(PROJECT_FLEXT_INFRA) release managed-git-tool --manifest "$$manifest" \
		$(if $(filter Y,$(APPLY)),--apply,)

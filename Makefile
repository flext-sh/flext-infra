...
_builtin_checkpoint_review: _builtin_require_environment
	...
	if git log -1 --format=%s | grep -q '^\[WIP\]'; then \
		printf 'ERROR: Review checkpoint requires a non-WIP HEAD\n' >&2; exit 2; \
	fi; \
	if [ "$$(gh pr view --json isDraft --jq .isDraft)" != false ]; then \
		printf 'ERROR: Review checkpoint requires a non-Draft PR\n' >&2; exit 2; \
	fi; \

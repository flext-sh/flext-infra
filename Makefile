...
CALLER_VIRTUAL_ENV_BIN := $(CALLER_VIRTUAL_ENV)/Scripts
CALLER_PATH_SEPARATOR := ;
else
RUNTIME_BIN := $(RUNTIME_VENV)/bin
RUNTIME_PYTHON := $(RUNTIME_BIN)/python
FLEXT_INFRA_RUNTIME_PYTHON := $(FLEXT_INFRA_RUNTIME_ROOT)/.venv/bin/python
CALLER_VIRTUAL_ENV_BIN := $(CALLER_VIRTUAL_ENV)/bin
CALLER_PATH_SEPARATOR := :
endif
SANITIZED_CALLER_PATH := $(CALLER_PATH)
ifneq ($(strip $(CALLER_VIRTUAL_ENV)),)
SANITIZED_CALLER_PATH := $(subst $(CALLER_VIRTUAL_ENV_BIN)$(CALLER_PATH_SEPARATOR),,$(SANITIZED_CALLER_PATH))
SANITIZED_CALLER_PATH := $(subst $(CALLER_PATH_SEPARATOR)$(CALLER_VIRTUAL_ENV_BIN),,$(SANITIZED_CALLER_PATH))
...
override PATH := $(RUNTIME_BIN)$(CALLER_PATH_SEPARATOR)$(SANITIZED_CALLER_PATH)
...
UV_RUN := env -u MYPYPATH -u VIRTUAL_ENV -u UV_PROJECT -u UV_PROJECT_ENVIRONMENT PATH="$(RUNTIME_BIN)$(CALLER_PATH_SEPARATOR)$(SANITIZED_CALLER_PATH)" PYTHONPATH="$(PROJECT_ROOT)/src"
PROJECT_INFRA_PYTHONPATH ?= $(MAKEFILE_ROOT)/src
PROJECT_FLEXT_INFRA := test -x "$(FLEXT_INFRA_PYTHON)" || { printf 'ERROR: FLEXT_INFRA_PYTHON must name an executable managed Python\n' >&2; exit 2; }; env -u PYTHONPATH -u MYPYPATH -u VIRTUAL_ENV -u UV_PROJECT -u UV_PROJECT_ENVIRONMENT PATH="$(dir $(FLEXT_INFRA_PYTHON))$(CALLER_PATH_SEPARATOR)$(SANITIZED_CALLER_PATH)" PYTHONPATH="$(PROJECT_INFRA_PYTHONPATH)" $(FLEXT_INFRA_PYTHON) -m flext_infra

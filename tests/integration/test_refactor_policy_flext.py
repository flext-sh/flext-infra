"""Integration tests for policy-driven FLEXT resolution."""

from __future__ import annotations

from flext_infra.refactor.flext_resolver import FlextInfraRefactorFLEXTResolver
from flext_tests import tm
from tests import c


class TestsFlextInfraIntegrationRefactorPolicyFlext:
    """Single top-level integration test class for FLEXT policy resolution."""
    ...
    def test_flext_resolver_accepts_expected_order(self) -> None:
        resolutions = FlextInfraRefactorFLEXTResolver.resolve(
            family_classes={
    ...
    def test_flext_resolver_rejects_wrong_order(self) -> None:
        raised = False
        try:
            FlextInfraRefactorFLEXTResolver.resolve(
                family_classes={
                    c.Infra.FacadeFamily.C: (self.DemoMigrationConstants),
                    c.Infra.FacadeFamily.T: self.DemoMigrationTypes,
                    c.Infra.FacadeFamily.P: (self.DemoMigrationProtocols),

...
def _write_workspace(tmp_path: Path) -> tuple[Path, tuple[str, ...]]:
    ...
    # These tests assert what the GENERATED Makefile contains, so planning the
    # rendered artifacts is sufficient and avoids unrelated filesystem writes.
    planned = tm.ok(
...
class TestsWorkspaceRootMakeContract:
    def test_workspace_root_make_template_is_owned_by_typed_config(self) -> None:
        ...
        tm.that("profiles" in type(make_entries[0]).model_fields, eq=False)

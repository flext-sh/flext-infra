...
class TestsWorkspaceRootMakeContract:
    ...
    def test_generated_make_exposes_typed_docs_lifecycle(self, tmp_path: Path) -> None:
        ...
        # As acoes de docs sao os proprios selectors do verbo: o bloco `docs`
        # guarda apenas a politica (quais mutam, onde reportar).
        docs_verb = next(
            verb for verb in config.Infra.codegen.make.verbs if verb.name == "docs"
        )
        docs_actions = docs_verb.whats
        ...
        for action in docs_actions:
            ...
            expected_actions = (
                tuple(item for item in docs_actions if item != docs_default_action)
                if action == docs_default_action
                else (action,)
            )
            ...
            elif action != docs_default_action:
                tm.that(output, lacks="--apply")

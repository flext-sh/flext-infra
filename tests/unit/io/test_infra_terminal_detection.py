...
class TestsFlextInfraIoInfraTerminalDetection:
    ...
    @contextmanager
    def _env(self, **updates: str | None) -> Generator[None]:
    ...
    def test_stream_reports_tty_state(self) -> None:
        tm.that(self._Stream(tty=True).isatty(), eq=True)
        tm.that(self._Stream(tty=False).isatty(), eq=False)

    def test_env_applies_and_restores_environment(self) -> None:
        os.environ["FLEXT_KEEP"] = "yes"
        with self._env(FLEXT_KEEP=None, FLEXT_TEST="1"):

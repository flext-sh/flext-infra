"""Tests for migrate-to-flext automation."""
...
class TestsFlextInfraRefactorInfraRefactorMigrateToClassFlext:
    """Behavior contract for test_infra_refactor_migrate_to_class_flext."""

    def test_migrate_to_flext_moves_constant_and_rewrites_reference(
        self, tmp_path: Path
    ) -> None:
        ...
        report = FlextInfraRefactorMigrateToClassFLEXT(workspace_root=project_root).run(
    ...
    def test_migrate_to_flext_inlines_alias_constant_into_constants_class(
        self, tmp_path: Path
    ) -> None:
        project_root = tmp_path / "sample"
        src_pkg = project_root / "src" / "sample_pkg"
    ...
    def test_migrate_to_flext_normalizes_facade_alias_to_c(
        self, tmp_path: Path
    ) -> None:
        project_root = tmp_path / "sample"
        src_pkg = project_root / "src" / "sample_pkg"
    ...
    def test_migrate_to_flext_rejects_unknown_target(self, tmp_path: Path) -> None:
        project_root = tmp_path / "sample"
        project_root.mkdir(parents=True)
        migrator = FlextInfraRefactorMigrateToClassFLEXT(workspace_root=project_root)
    ...
    def test_flext_scan_respects_namespace_scan_dirs_src_only(
        self, tmp_path: Path
    ) -> None:
        project_root = tmp_path / "sample"
        src_pkg = project_root / "src" / "sample_pkg"
    ...
    def test_migrate_to_flext_moves_manual_uppercase_assignment(
        self, tmp_path: Path
    ) -> None:
        project_root = tmp_path / "sample"
        src_pkg = project_root / "src" / "sample_pkg"
    ...
    def test_migrate_to_flext_is_idempotent_on_second_run(self, tmp_path: Path) -> None:
        ...
        service = FlextInfraRefactorMigrateToClassFLEXT(workspace_root=project_root)

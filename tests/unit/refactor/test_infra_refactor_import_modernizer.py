...
class TestsFlextInfraRefactorInfraRefactorImportModernizer:
    ...
    def test_modernize_orchestrator_dry_run_reports_planned_modification(
        ...
        tm.ok(result)
        tm.that(result.value.modified, eq=True)
        tm.that(result.value.refactored_code, eq="value = 'new_value'\n")
        tm.that(source_path.read_text(encoding="utf-8"), eq="value = 'old_value'\n")
    ...
    def test_modernize_orchestrator_returns_result_failure_for_transform_error(
        ...
        tm.fail(result)
        tm.that(result.error_code, eq="MODERNIZE_TRANSFORM_FAILED")

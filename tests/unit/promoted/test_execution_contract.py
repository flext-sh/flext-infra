...
class TestsFlextInfraPromotedExecutionContract:
    ...
    class TestsFlextInfraPromotedDispatchAlwaysExecutes:
        ...
        def _write_registry(tmp_path: Path) -> t.Pair[Registry, Path]:
            ...
            registry = Registry()
            # Use the public alias contract instead of replacing private settings state.
            command = u.Tests.promoted_command(path=command_path).model_copy(
                update={"aliases": ("execute",)}
            )
        ...
        def test_dispatch_executes_declared_operation(
            ...
            with tm.scope(
                ...
                exit_code = dispatch(registry, "execute")
                assert exit_code == 0

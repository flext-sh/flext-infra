...
"""
    ...
    def _write_command(self, tmp_path: Path, source: str) -> Path:
        ...
        (venv_bin / "python").symlink_to(self._PYTHON)
        return command

    def _read_ready(
        self, streams: Mapping[int, str], timeout: float | None = None
    ) -> tuple[str, str]:
        if timeout is None:
    ...
    def _spawn_probe(
        ...
        *,
        ...
        argv = (self._PYTHON, "-P", "-c", self._PROBE, command)
        if process_group:
            return os.posix_spawn(
                self._PYTHON, argv, env, file_actions=file_actions, setpgroup=0
            )
        return os.posix_spawn(self._PYTHON, argv, env, file_actions=file_actions)
    ...
    def test_run_streams_stdout_and_stderr_before_child_completion(
        ...
        reaped = False
        try:
            ready, _, _ = select.select((ready_read,), (), (), self._BARRIER_TIMEOUT)
            if not ready:
    ...
    def test_run_returns_exact_nonzero_exit_status(self, tmp_path: Path) -> None:
        ...
        """
        command = self._write_command(
            tmp_path, f"raise SystemExit({self._NONZERO_EXIT})\n"
        )
        exit_code = self._wait_status(self._spawn_probe(command, env=self._probe_env()))
        if exit_code != self._NONZERO_EXIT:
            pytest.fail(f"expected {self._NONZERO_EXIT}, got {exit_code}")

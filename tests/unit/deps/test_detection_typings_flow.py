...
class TestsFlextInfraDepsDetectionTypingsFlow:
    def test_module_to_types_package(self) -> None:
        ...
        tm.that(service.module_to_types_package("yaml", {
            "typing_libraries": {"module_to_package": {"yaml": "custom-types-yaml"}}
        }), eq="custom-types-yaml")

    def test_custom_typings_and_managed_dev_are_both_available(self, tmp_path: Path) -> None:
        ...
        service = FlextInfraDependencyDetectionService()
        tm.that(service.get_current_typings_from_pyproject(tmp_path), eq=["pytest", "types-python-dateutil", "types-pyyaml", "types-requests"])
        tm.that(service.get_current_typings_from_pyproject(tmp_path, include_dev=False), eq=["types-pyyaml", "types-requests"])
        limits = tmp_path / "limits.toml"
        limits.write_text("[typing_libraries]\n", encoding="utf-8")
        report = tm.ok(service.get_required_typings(tmp_path, limits, include_mypy=False))
    ...
    def test_retired_poetry_typings_are_not_a_source(self, tmp_path: Path) -> None:
        ...
        tm.that(FlextInfraDependencyDetectionService().get_current_typings_from_pyproject(tmp_path), empty=True)
    ...
    def test_invalid_typings_table_fails_at_ingress(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(c.ValidationError):
            FlextInfraDependencyDetectionService().get_current_typings_from_pyproject(tmp_path)

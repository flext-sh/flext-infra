...
class TestsFlextInfraDepsModernizerPyright:
    ...
    def _pyright_baseline(self, doc: t.Cli.TomlDocument) -> t.JsonMapping | None:
        """Unwrap the rendered pyright table and assert its runtime-owned baseline."""
        tables = self._pyright_tables(doc)
        if tables is None:
            return None
        _, pyright = tables
        self._assert_runtime_owned_virtualenv(pyright)
    ...
    def test_root_config_includes_member_src_paths(
        ...
        if src_env is None:
            return
        extra_paths = u.Cli.toml_unwrap_item(

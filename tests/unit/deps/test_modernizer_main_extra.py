...
class TestsFlextInfraDepsModernizerMainExtra:
    ...
    @pytest.mark.parametrize(
        "content",
        [
            pytest.param(None, id="missing-root-pyproject"),
            pytest.param("[invalid toml {", id="invalid-root-pyproject"),
        ],
    )
    def test_run_handles_root_edge_cases(
        self, tmp_path: Path, content: str | None
    ) -> None:
        """Return a failing command status for absent or invalid root TOML."""
    ...
    def test_run_rejects_empty_root_project_contract(self, tmp_path: Path) -> None:
        """Let the declaration owner expose its precise missing-name error."""
        workspace = tmp_path / "workspace"
        workspace.mkdir()
        (workspace / c.Infra.PYPROJECT_FILENAME).write_text("", encoding="utf-8")
        modernizer = FlextInfraPyprojectModernizer(workspace_root=workspace)

        with pytest.raises(ValueError, match=r"missing or empty \[project\]\.name"):
            modernizer.run()

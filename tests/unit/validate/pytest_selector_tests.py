...
class TestsFlextInfraPytestSelectorValidator:
    ...
    def test_file_rejects_non_normalized_or_control_text(self, file: str) -> None:
        with pytest.raises(c.ValidationError, match="file must"):
            FlextInfraPytestSelectorValidator(workspace=Path.cwd(), file=file)

    def test_what_accepts_only_canonical_test_modes(self) -> None:
        """Keep test mode selection inside the typed canonical launcher."""
        for what in ("all", "profile"):
            validator = FlextInfraPytestSelectorValidator(
                workspace=Path.cwd(), what=what
            )
            tm.ok(validator.execute())
        with pytest.raises(c.ValidationError, match="what must be one of"):
            FlextInfraPytestSelectorValidator(
                workspace=Path.cwd(), what="$(shell touch marker)"
            )
    ...
    def test_file_rejects_symlink_hop(self, tmp_path: Path) -> None:
        ...
        result = FlextInfraPytestSelectorValidator(
            workspace=tmp_path, file="tests/linked.py"
        ).execute()
    ...
    def test_missing_file_fails_at_typed_boundary(self, tmp_path: Path) -> None:
        result = FlextInfraPytestSelectorValidator(
            workspace=tmp_path, file="tests/missing.py::test_missing"
        ).execute()

        tm.fail(result, has="does not exist")

    def test_validate_route_uses_selector_owner(self) -> None:
        routes = {
            route.name: route.model_cls
            for route in ValidationCommandRoutes.validate_command_routes
        }

        tm.that(routes["pytest-selector"], eq=FlextInfraPytestSelectorValidator)

...
class TestsVscodeOwnerMerge:
    ...
    def test_merge_marks_drift_and_renders_canonical_content(
        ...
        rendered = tm.ok(FlextInfraCodegen.render_vscode_settings(root))
        doc = t.Cli.JSON_MAPPING_ADAPTER.validate_python(
            tm.ok(u.Cli.json_parse(rendered))
        )
    ...
    def test_merge_reaches_fixed_point_after_apply(self, tmp_path: Path) -> None:
        ...
        settings_path.parent.mkdir(parents=True)
        _ = settings_path.write_text("{}\n", encoding="utf-8")
        first = tm.ok(FlextInfraCodegen.render_vscode_settings(root))
        _ = settings_path.write_text(first, encoding="utf-8")
        second = tm.ok(FlextInfraCodegen.render_vscode_settings(root))

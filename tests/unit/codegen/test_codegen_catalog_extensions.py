...
class TestsCodegenCatalogExtensions:
    def test_bootstrap_toolchain_uses_bounded_minor_selector(self) -> None:
        mise_parts = config.Infra.codegen.toolchain.mise_version.split(".")
        tm.that(len(mise_parts), eq=2)
    ...
    def test_local_manifest_conforms_without_global_repository_rows(
        ...
        tm.that(
            any("beads" in selector for selector in mise["tools"]),
            eq=False,
            msg="generated consumers must not inherit flext-infra's Beads pin",
        )

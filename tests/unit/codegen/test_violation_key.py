...
class TestsFlextInfraCodegenViolationKey:
    ...
    _SOURCE_10: ClassVar[t.StrSequence] = tuple(f"line {i}" for i in range(10))
    ...
    def test_from_violation_produces_content_hash(self) -> None:
        ...
        v = self._violation(module="src/models.py", rule="NS-002", line=5)
        key = m.Infra.ViolationKey.from_violation(v, self._SOURCE_10)
    ...
    def test_content_hash_stable_across_calls(self) -> None:
        ...
        v = self._violation(line=4)
        key_a = m.Infra.ViolationKey.from_violation(v, self._SOURCE_10)
        key_b = m.Infra.ViolationKey.from_violation(v, self._SOURCE_10)
    ...
    def test_content_hash_changes_with_context(self) -> None:
        ...
        key_original = m.Infra.ViolationKey.from_violation(v, self._SOURCE_10)
    ...
    def test_frozenset_reconciliation(self) -> None:
        ...
        v1 = self._violation(module="a.py", rule="NS-001", line=2)
        v2 = self._violation(module="b.py", rule="NS-002", line=3)
        v3 = self._violation(module="c.py", rule="NS-003", line=4)

        before = frozenset({
            m.Infra.ViolationKey.from_violation(v1, self._SOURCE_10),
            m.Infra.ViolationKey.from_violation(v2, self._SOURCE_10),
            m.Infra.ViolationKey.from_violation(v3, self._SOURCE_10),
        })

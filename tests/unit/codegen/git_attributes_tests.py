...
class TestsGeneratedGitAttributes:
    ...
    @pytest.mark.parametrize(
        "generated", ["generated output.txt", "geração&output.txt", "literal[*?].txt"]
    )
    def test_git_consumes_exact_generated_paths(
        ...
        rendered = tm.ok(
            u.Cli.template_render(
                ...
                / codegen.make.git_attributes_template,
                m.Infra.MakeWorkflowRenderSpec(
                    dist=root.name,
                    make=codegen.make,
                    generated_paths=(u.Infra.git_attribute_pattern(f"/{generated}"),),
                ),
            )
        )
        ...
        tm.ok(
            u.Cli.atomic_write_text_file(
                root / c.Infra.GITATTRIBUTES_FILENAME, rendered
            )
        )
        observed = tm.ok(
            u.Cli.run(
    ...
    def test_unchanged_checkpoint_preserves_head(self, tmp_path: Path) -> None:
        ...
        root = test_u.Tests.git_repository(tmp_path)
        before = tm.ok(
            u.Cli.run([c.Infra.GIT, "rev-parse", "HEAD"], cwd=root)
        ).stdout.strip()
        checkpoint = tm.ok(u.Infra.git_checkpoint_worktree(root, message="checkpoint"))
        repeated = tm.ok(
            u.Infra.git_checkpoint_worktree(root, message="checkpoint again")
        )
        after = tm.ok(
            u.Cli.run([c.Infra.GIT, "rev-parse", "HEAD"], cwd=root)
        ).stdout.strip()

...
class TestsFlextInfraCodegenLazyInitService:
    ...
    def test_applied_initializer_is_valid_and_byte_idempotent(
        self, tmp_path: Path
    ) -> None:
        """Generation owns valid Python and converges without invoking Ruff."""
        workspace_root, package_root = u.Tests.create_lazy_init_workspace(tmp_path)
        u.Tests.write_lazy_init_namespace_module(
            package_root / "models.py", class_name="FlextTestsModels", alias="m"
        )
        ...
        checked = u.Tests.create_lazy_init_service(workspace_root)
        checked.target_module = "flext_test_project"
        check_result = checked.generate_inits(check_only=True)

        tm.that(result.success, eq=True)
        tm.that(service.modified_files, eq=(str(init_path),))
        compile(init_path.read_text(encoding="utf-8"), str(init_path), "exec")
        tm.that(check_result, eq=0)
        tm.that(checked.modified_files, eq=())

...
class TestGitHookConformance:
    ...
    def test_member_hook_config_is_retired_by_conform(self, tmp_path: Path) -> None:
        """A workspace member carries no CI matrix workflow, and conform removes one."""
        root = tmp_path / "flext-member"
        root.mkdir()
        ci_matrix = root / ".github" / "workflows" / "ci-matrix.yml"
        ci_matrix.parent.mkdir(parents=True)
        ci_matrix.write_text(
        ...
        tm.that(ci_matrix in retired, eq=True)

...
class TestsFlextInfraCodegenInit:
    """Test suite for flext_infra.codegen module initialization."""
    ...
    def _baseline_leaf_modules(self) -> t.VariadicTuple[str]:
        ...
        repository_root = self._PROJECT_SRC.parent.parent
        sibling_srcs = sorted(
            str(member / "src")
        ...
        probe_env["PYTHONPATH"] = os.pathsep.join([
            *sibling_srcs,
            str(self._PROJECT_SRC),
        ])
        from tests import u

        result = tm.ok(
            u.Cli.run([sys.executable, "-c", code], env=probe_env, timeout=60)
        )
        return tuple(
            line for line in result.stdout.strip().splitlines() if line.strip()
        )

    def test_codegen_getattr_raises_attribute_error(self) -> None:
        """Test that accessing nonexistent attribute raises AttributeError."""
        with pytest.raises(AttributeError):
            _ = getattr(codegen_module, self._ABSENT_SYMBOL)

    def test_codegen_package_does_not_reexport_leaf_implementations(self) -> None:
        ...
        tm.that(
            codegen_module.FlextInfraCodegenConform.__name__,
            eq="FlextInfraCodegenConform",
        )

...
class TestCodegenManifestlessExisting:
    def test_existing_root_uses_pep621_metadata_for_managed_artifacts(
        ...
        pyproject_plan: m.Infra.CodegenPlan = tm.ok(
            FlextInfraCodegenConform(workspace_root=root).plan(request)
        )
        tm.ok(FlextInfraCodegenPublisher.apply(pyproject_plan))
        ...
        for relative, content in preserved.items():
            tm.that(relative not in plans, eq=True)
        ...
        for excluded in (
            ...
            tm.that(excluded not in plans, eq=True)
        for required in (".mise.toml", ".python-version", ".gitignore"):
            tm.that(plans[required].changed, eq=True)

        tm.ok(FlextInfraCodegenPublisher.apply(initial_plan))

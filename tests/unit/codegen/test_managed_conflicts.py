...
class TestsFlextInfraManagedConflictRecovery:
    ...
    def test_every_table_the_conform_pipeline_writes_is_recoverable(self) -> None:
        ...
        The tables are named by the same constants the conform pipeline writes
        through, so the writer and this declaration cannot drift. They did
        drift once: the pipeline gained the namespace and budget tables while
        the declaration did not, and absorbing the integration base then
        dead-ended the superproject merge on the owner's own output.
        """
        pyproject = tm.ok(u.Infra.pyproject_managed_file())

        for table in (c.Infra.CONFORM_NAMESPACE_TABLE, c.Infra.CONFORM_BUDGET_TABLE):
            tm.that(
                u.Infra.toml_section_is_owned(
                    u.Cli.toml_dot_path(*table), pyproject.conflict_sections
                ),
                eq=True,
            )

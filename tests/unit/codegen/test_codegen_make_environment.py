...
# mro-izia.1 (agent kimi): a fixture project generated by this module runs its
# OWN make, so nothing about the outer invocation may reach it — neither the
# environment nor the selection smuggled through GNUMAKEFLAGS/MAKEFLAGS. The
# orchestrator SSOT already enumerates exactly that set; `UV` is added because
# these cases bind their own uv shim.
_NESTED_MAKE_REMOVE_KEYS: frozenset[str] = frozenset({
    ...
    def test_setup_provisions_environment_before_project_runtime(
        ...
        clean_env = {
            **{
                ...
                if key not in _NESTED_MAKE_REMOVE_KEYS
            },
        ...
        result = u.Cli.run_raw(
            ...
            cwd=project_root,
            env=clean_env,
            remove_env_keys=tuple(_NESTED_MAKE_REMOVE_KEYS),
        )
    ...
    def test_dependency_upgrade_selects_only_one_distribution(
        ...
        process = tm.ok(
            u.Cli.run_raw(
                ...
                remove_env_keys=tuple(_NESTED_MAKE_REMOVE_KEYS - {"UV"}),
            )
        )

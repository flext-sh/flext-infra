...
class TestCodegenCiMatrix:
    ...
    @pytest.mark.parametrize(
        ("provenance", "expected_absent"),
        [
            pytest.param("# [MANAGED] flext_infra codegen\n", 1, id="managed"),
            pytest.param(
                "# Generated by flext_infra codegen\n", 0, id="legacy-unmanaged"
            ),
            pytest.param("", 0, id="repository-owned"),
        ],
    )
    def test_profile_excluded_workflow_requires_managed_provenance_for_removal(
        self, tmp_path: Path, provenance: str, expected_absent: int
    ) -> None:
        """Delete only profile-excluded workflows carrying [MANAGED] provenance."""
        from flext_infra import m
        ...
        tm.that(len(absent), eq=expected_absent)
        if absent:

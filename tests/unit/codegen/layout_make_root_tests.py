"""Regression tests for checkout-derived WORKSPACE_ROOT in the flext-infra Makefile.
...
def _make_database_workspace_root(*extra_args: str, env: dict[str, str]) -> str:
    """Read WORKSPACE_ROOT from the flext-infra make database."""
    ...
    return next(
        ...
        if line.startswith(("WORKSPACE_ROOT =", "WORKSPACE_ROOT :="))
    )


def test_make_workspace_root_ignores_foreign_env_leak(tmp_path: Path) -> None:
...
def test_make_workspace_root_honors_command_line_override(tmp_path: Path) -> None:
    """An explicit command-line WORKSPACE_ROOT stays authoritative."""
    override = str(tmp_path / "explicit-target")
    resolved = _make_database_workspace_root(
        f"WORKSPACE_ROOT={override}", env=dict(os.environ)
    )

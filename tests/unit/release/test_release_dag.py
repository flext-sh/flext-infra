...
class TestsFlextInfraReleaseDag:
    ...
    def test_release_succeed_in_dry_run_mode(self, tmp_path: Path) -> None:
    ...
    def test_release_selected_validate_phase_skips_release_artifacts(
        ...
        result = FlextInfraReleaseOrchestrator().run_release(
            _make_config(workspace, phases=[c.Infra.VERB_VALIDATE], dry_run=True)
        )
    ...
    def test_release_build_failure_propagates_from_real_make(
        ...
        workspace = u.Tests.create_release_workspace(tmp_path, root_build_exit_code="1")
        result = FlextInfraReleaseOrchestrator().run_release(
            _make_config(workspace, phases=[c.Infra.DIR_BUILD], dry_run=False)
        )

        tm.fail(result)
        tm.that((result.error or ""), has="build failed")

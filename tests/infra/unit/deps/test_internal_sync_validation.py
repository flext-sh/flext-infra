...
class TestFlextInfraInternalDependencySyncService:
    ...
    def test_validate_repo_url_invalid(self) -> None:
        u.Tests.Matchers.fail(
            FlextInfraInternalDependencySyncService.validate_repo_url("not-a-url")
        )
    ...
    def test_ssh_to_https_conversion(self) -> None:
        ...
        u.Tests.Matchers.that(result.startswith("https://"), eq=True)
        u.Tests.Matchers.that(result, contains="flext-sh/flext")

    def test_ssh_to_https_already_https(self) -> None:
        url = "https://github.com/flext-sh/flext.git"
        u.Tests.Matchers.that(
            FlextInfraInternalDependencySyncService.ssh_to_https(url), eq=url
        )
...
class TestValidateGitRefEdgeCases:
    ...
    def test_valid_refs(self, ref: str) -> None:
        u.Tests.Matchers.ok(
            FlextInfraInternalDependencySyncService.validate_git_ref(ref)
        )

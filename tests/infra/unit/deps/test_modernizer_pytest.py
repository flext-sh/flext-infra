...
class TestEnsurePytestConfigPhase:
    ...
    def test_ensure_pytest_config_sets_fields(self) -> None:
        ...
        u.Tests.Matchers.that(any("minversion" in c for c in changes), eq=True)
        u.Tests.Matchers.that(any("python_classes" in c for c in changes), eq=True)
        u.Tests.Matchers.that(any("python_files" in c for c in changes), eq=True)
        u.Tests.Matchers.that(any("addopts" in c for c in changes), eq=True)
        u.Tests.Matchers.that(any("markers" in c for c in changes), eq=True)
    ...
    def test_ensure_pytest_config_preserves_existing(self) -> None:
        ...
        u.Tests.Matchers.that(isinstance(pytest_section, MutableMapping), eq=True)
        if not isinstance(pytest_section, MutableMapping):
            return
        ini_options = pytest_section["ini_options"]
        u.Tests.Matchers.that(isinstance(ini_options, MutableMapping), eq=True)
...
def test_ensure_pytest_config_phase_apply_minversion() -> None:
    ...
    changes = EnsurePytestConfigPhase(_test_tool_config()).apply(doc)
    u.Tests.Matchers.that(any("minversion set to 8.0" in c for c in changes), eq=True)
    ini_options = pytest_section["ini_options"]
    u.Tests.Matchers.that(isinstance(ini_options, MutableMapping), eq=True)
    if isinstance(ini_options, MutableMapping):
        u.Tests.Matchers.that(
...
def test_ensure_pytest_config_phase_apply_python_classes() -> None:
    ...
    pytest_section["ini_options"] = tomlkit.table()
    changes = EnsurePytestConfigPhase(_test_tool_config()).apply(doc)
    u.Tests.Matchers.that(any("python_classes updated" in c for c in changes), eq=True)

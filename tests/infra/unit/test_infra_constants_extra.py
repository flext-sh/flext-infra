...
class TestFlextInfraConstantsCheckNamespace:
    ...
    def test_default_check_dirs_is_list(self) -> None:
        u.Tests.Matchers.that(infra_c.Infra.DEFAULT_CHECK_DIRS, is_=list)

    def test_default_check_dirs_contains_standard_dirs(self) -> None:
        dirs = infra_c.Infra.DEFAULT_CHECK_DIRS
        u.Tests.Matchers.that(dirs, contains="src")
        u.Tests.Matchers.that(dirs, contains="tests")
        u.Tests.Matchers.that(dirs, contains="examples")
        u.Tests.Matchers.that(dirs, contains="scripts")

    def test_check_dirs_subproject_is_list(self) -> None:
        u.Tests.Matchers.that(infra_c.Infra.CHECK_DIRS_SUBPROJECT, is_=list)
    ...
    def test_check_dirs_are_strings(self) -> None:
        for d in infra_c.Infra.DEFAULT_CHECK_DIRS:
            u.Tests.Matchers.that(d, is_=str)
...
class TestFlextInfraConstantsGithubNamespace:
    ...
    def test_github_repo_name_constant(self) -> None:
        u.Tests.Matchers.that(infra_c.Infra.GITHUB_REPO_NAME, eq="flext-sh/flext")

    def test_github_constants_are_strings(self) -> None:
        u.Tests.Matchers.that(infra_c.Infra.GITHUB_REPO_URL, is_=str)
        u.Tests.Matchers.that(infra_c.Infra.GITHUB_REPO_NAME, is_=str)
...
class TestFlextInfraConstantsEncodingNamespace:
    ...
    def test_default_encoding_constant(self) -> None:
        u.Tests.Matchers.that(infra_c.Infra.Encoding.DEFAULT, eq="utf-8")

    def test_encoding_constant_is_string(self) -> None:
        u.Tests.Matchers.that(infra_c.Infra.Encoding.DEFAULT, is_=str)
...
class TestFlextInfraConstantsAlias:
    ...
    def test_c_alias_is_string(self) -> None:
        u.Tests.Matchers.that(infra_c, is_=type)

    def test_c_alias_provides_access_to_namespaces(self) -> None:
        u.Tests.Matchers.that(hasattr(infra_c, "Infra"), eq=True)
        u.Tests.Matchers.that(hasattr(infra_c.Infra, "Paths"), eq=True)
        u.Tests.Matchers.that(hasattr(infra_c.Infra, "Files"), eq=True)
        u.Tests.Matchers.that(hasattr(infra_c.Infra, "Gates"), eq=True)
        u.Tests.Matchers.that(hasattr(infra_c.Infra, "Status"), eq=True)
        u.Tests.Matchers.that(hasattr(infra_c.Infra, "Excluded"), eq=True)
        u.Tests.Matchers.that(hasattr(infra_c.Infra, "Check"), eq=True)
        u.Tests.Matchers.that(hasattr(infra_c.Infra, "Github"), eq=True)
        u.Tests.Matchers.that(hasattr(infra_c.Infra, "Encoding"), eq=True)
...
class TestFlextInfraConstantsImmutability:
    ...
    def test_excluded_dirs_are_immutable(self) -> None:
        ...
        u.Tests.Matchers.that(hasattr(excluded, "add"), eq=False)

    def test_check_dirs_are_immutable(self) -> None:
        dirs = infra_c.Infra.DEFAULT_CHECK_DIRS
        u.Tests.Matchers.that(hasattr(dirs, "append"), eq=False)
...
class TestFlextInfraConstantsConsistency:
    ...
    def test_all_status_values_are_uppercase(self) -> None:
        u.Tests.Matchers.that(infra_c.Infra.Status.PASS.isupper(), eq=True)
        u.Tests.Matchers.that(infra_c.Infra.Status.FAIL.isupper(), eq=True)
        u.Tests.Matchers.that(infra_c.Infra.Status.OK.isupper(), eq=True)
        u.Tests.Matchers.that(infra_c.Infra.Status.WARN.isupper(), eq=True)
    ...
    def test_all_gate_values_are_lowercase(self) -> None:
        ...
        for gate in gates:
            u.Tests.Matchers.that(
                gate.islower(), eq=True, msg=f"Gate {gate} should be lowercase"
            )
    ...
    def test_excluded_dirs_no_duplicates(self) -> None:
        ...
        u.Tests.Matchers.that(len(common), eq=len(set(common)))
        u.Tests.Matchers.that(len(doc), eq=len(set(doc)))

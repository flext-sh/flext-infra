...
|---|---|---|---|
| Fleet toolchain (python, uv, kubectl, helm, kind, taplo, ast-grep, gitleaks, scc, kubeconform, qlty, node + jscpd for the `duplication` check gate, go, beads) | `config/codegen.yaml` `toolchain:` | `make gen` → `.mise.toml`, `mise.lock`, `bin/mise` | direnv activation of the project root |

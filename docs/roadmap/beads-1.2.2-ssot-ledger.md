...
| 2026-08-28T12:28:00-03:00 | `af09dd31f` | Replacement PR remote gates | RED: blocking CI could not provision `.venv/bin/python` because the clean checkout received a tracked absolute `.venv` symlink; Sonar reported 18 owner-level maintainability/security issues | Repair the tracked-environment leak and the shared generated/Python owners, then repeat every remote and local gate; approval remains pending |
| 2026-08-28T12:32:36-03:00 | worktree | Clean-checkout CI owner repair | PASS: `.venv` removed only from the Git index and remains locally intact/ignored; 18 focused renderer tests passed; first apply changed exactly the workflow plus five Docker projections; second apply changed 0; `gen check` passed | Docker daemon was unavailable locally, so image execution is not claimed; the remote runner must prove the generated image while the remaining Python Sonar findings are repaired |
| 2026-08-28T12:38:35-03:00 | worktree | First static gate after Sonar-owner simplification | ENVIRONMENTAL RED before analysis: public import raised `MemoryError` while Pydantic generated a dependency schema | Host still reported 29 GiB available but another preserved process held about 9 GiB; no unrelated process was stopped and the gate was repeated |
| 2026-08-28T12:39:22-03:00 | worktree | Repeated full `make check` | RED: Pyrefly and Mypy each rejected the normalized lock entry's abstract `Mapping` at the concrete JSON-list boundary; the other five stages passed | Materialize the already-validated mapping as `dict` at that boundary and repeat all seven stages |
| 2026-08-28T12:40:08-03:00 | worktree | Full `make check` after concrete JSON boundary | PASS: lint, smells, Markdown, Pyrefly, Mypy, security, and Pyright reported 0 errors and 0 skips | All six Python Sonar owners were simplified without changing public behavior; focused lock/environment/topology contracts also passed 8, 10, and 14 complete cases respectively |

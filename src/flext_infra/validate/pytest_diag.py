...
class FlextInfraPytestDiagExtractor(FlextInfraPytestDiagXmlMixin, s[bool]):
    ...
    @staticmethod
    def _extract_slow_from_log(
        lines: t.StrSequence, diag: m.Infra.DiagResult
    ) -> None:
    ...
    def _extract_diagnostics(
        ...
        xml_result = self._parse_xml(junit_path, diag)
        if xml_result.failure:
            return r[m.Infra.PytestDiagnostics].fail(
                xml_result.error or f"Failed to parse JUnit XML: {junit_path}"
            )
        if not xml_result.value:

...
class FlextInfraEnforcementFixerOrchestrator(
    ...
    def _selected_rules(
        self, catalog: core_m.EnforcementCatalog | None = None
    ...
    def _has_adapter(self, rule: core_m.EnforcementRuleSpec) -> bool:
    ...
    def _fix_project(
        ...
        rules: t.SequenceOf[core_m.EnforcementRuleSpec],
    ...
    def _group_by_adapter(
        self, rules: t.SequenceOf[core_m.EnforcementRuleSpec]
    ) -> tuple[
        dict[type[FlextInfraFixerAdapter], list[core_m.EnforcementRuleSpec]],
        tuple[core_m.EnforcementRuleSpec, ...],
    ]:
        """Group rules by the adapter that can handle their fix_action."""
        grouped: dict[
            type[FlextInfraFixerAdapter], list[core_m.EnforcementRuleSpec]
        ] = defaultdict(list)
        unhandled: list[core_m.EnforcementRuleSpec] = []
        for rule in rules:
            fix_action = rule.fix_action
            if fix_action is None:
    ...
    def _adapter_for(
        self, fix_action: core_m.EnforcementFixAction
    ...
    def _collect_violations(
        self, project_dir: Path, rules: t.SequenceOf[core_m.EnforcementRuleSpec]
    ) -> tuple[

...
class FlextInfraMROCompletenessDetector(FlextInfraScanFileMixin, p.Infra.Scanner):
    ...
    def detect_file(
        ...
        facade = u.find_facade_alias(res, family)
        if facade is None:
            ...
            if suffix:
                facade = next(
                    (
                        n
                        for n in u.get_module_classes(rope_project, res)
        ...
        for path in scan_paths:
            r = u.get_resource_from_path(rope_project, path)
            if r is None:
                continue
            for ci in u.get_class_info(rope_project, r):
                if ci.name == facade:
                    declared = set(ci.bases)
                elif not ci.name.startswith("_") and ci.name.startswith(facade):
                    expected[ci.name] = ci.line
        root = u.resolve_project_root(file_path)
        if root is not None:
            for base in u.build_expected_base_chains(project_root=root).get(family, []):

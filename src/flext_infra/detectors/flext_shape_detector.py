"""Detect FLEXT-shape violations (ENFORCE-046/047/049/051) via rope.
...
class FlextInfraFLEXTShapeDetector:
    """Detect FLEXT-shape violations using rope structural/semantic analysis."""
    ...
    def detect_file(
    ...
    ) -> t.SequenceOf[m.Infra.FLEXTShapeViolation]:
        ...
        if res is None:
            FlextInfraFLEXTShapeDetector._record_parse_failure(
                ctx,
        ...
        parent_map = FlextInfraFLEXTShapeDetector._build_parent_map(tree)
        class_nodes = FlextInfraFLEXTShapeDetector._collect_class_nodes(
            tree, parent_map
        )
        package_name = u.Infra.package_name(file_path).split(".", 1)[0]
        project_name = ctx.project_name or package_name.replace("_", "-")
        project_prefix = FlextInfraFLEXTShapeDetector._project_prefix(project_name)
        valid_suffixes = FlextInfraFLEXTShapeDetector._valid_alias_suffixes(
            package_name
        )
        violations: list[m.Infra.FLEXTShapeViolation] = []
        for node, qualname in class_nodes:
            bases = FlextInfraFLEXTShapeDetector._class_bases(node)
            if not bases:
                continue
            first_base = bases[0]
            ...
            if is_nested:
                violation = FlextInfraFLEXTShapeDetector._check_enforce_046(
                    node=node,
                    qualname=qualname,
                ...
                continue
            utility_check = FlextInfraFLEXTShapeDetector._check_enforce_051(
                node=node,
            ...
            if utility_check is not None:
                ...
                continue
            facade_check = FlextInfraFLEXTShapeDetector._check_enforce_047_049(
                node=node,
                class_name=class_name,
                bases=bases,
                first_base=first_base,
    ...
    def _record_parse_failure(
        ...
        """Record FLEXT-shape parse failures, or fail loud without a collector."""
        if ctx.parse_failures is None:
            msg = f"flext_shape detector could not parse {ctx.file_path}: {detail}"
    ...
    def _alias_base_set(
        ...
        ancestors = {
            ...
            for superclass in FlextInfraFLEXTShapeDetector._all_superclasses(obj)
        }
    ...
    def _single_peer_base_allowed(
        *,
        ...
        if not FlextInfraFLEXTShapeDetector._is_facade(class_name, project_prefix):
            return False
        unparametrized = first_base.split("[", 1)[0]
        if not unparametrized.startswith(project_prefix):
            return False
        if unparametrized.endswith(valid_suffixes):
            return False
        return bool(
            FlextInfraFLEXTShapeDetector._alias_base_set(
    ...
    def _peer_first_allowed(
        *,
        ...
        if not FlextInfraFLEXTShapeDetector._is_facade(class_name, project_prefix):
            return False
        if len(bases) < FlextInfraFLEXTShapeDetector._BINARY_ARITY:
            return False
        unparametrized = first_base.split("[", 1)[0]
        if not unparametrized.startswith(project_prefix):
            return False
    ...
    def _all_superclasses(
        ...
        for superclass in direct:
            result.extend(
                FlextInfraFLEXTShapeDetector._all_superclasses(
    ...
    def _check_enforce_047_049(
        *,
        ...
        pymodule: t.Infra.RopePyModule,
        file_path: Path,
    ) -> m.Infra.FLEXTShapeViolation | None:
        """Return ENFORCE-047/049 violation when facade first base is invalid."""
        if not FlextInfraFLEXTShapeDetector._is_facade(class_name, project_prefix):
            return None
        unparametrized = first_base.split("[", 1)[0]
        if FlextInfraFLEXTShapeDetector._is_utilities_self_root(
            file_path, unparametrized
        ):
            return None
        if FlextInfraFLEXTShapeDetector._is_alias_or_service_base(
            unparametrized, valid_suffixes
        ):
            return None
        if FlextInfraFLEXTShapeDetector._is_service_alias_base(
            first_base, project_prefix, pymodule
        ):
            return None
        if len(bases) == 1 and FlextInfraFLEXTShapeDetector._single_peer_base_allowed(
            class_name=class_name,
            first_base=first_base,
            project_prefix=project_prefix,
            valid_suffixes=valid_suffixes,
            ...
            return None
        if len(bases) >= FlextInfraFLEXTShapeDetector._BINARY_ARITY and (
            FlextInfraFLEXTShapeDetector._peer_first_allowed(
                class_name=class_name,
                bases=bases,
                first_base=first_base,
                project_prefix=project_prefix,
        ...
        return m.Infra.FLEXTShapeViolation(
            file=str(file_path),
    ...
    def _check_enforce_046(
        *, node: object, qualname: str, first_base: str, file_path: Path
        ...
        if first_base_qualname != outer_name:
            return None
        if not FlextInfraFLEXTShapeDetector._has_only_dunder_attrs(node):
            return None
        line = getattr(node, "lineno", 1)
        detail = (
    ...
    def _check_enforce_051(
        *,
        ...
        if len(bases) < FlextInfraFLEXTShapeDetector._BINARY_ARITY:
            return None
        if not FlextInfraFLEXTShapeDetector._is_utilities_self_root(
            file_path, first_base
        ):
            return None
        if not FlextInfraFLEXTShapeDetector._class_body_uses_name(node, "u"):
            return None
        line = getattr(node, "lineno", 1)
        detail = (
...
__all__: list[str] = ["FlextInfraFLEXTShapeDetector"]

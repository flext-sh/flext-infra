"""Detect facade classes missing FLEXT composition bases via rope.
...
class FlextInfraFLEXTCompletenessDetector:
    """Detect facade classes missing FLEXT bases via rope."""

    @staticmethod
    def detect_file(
        ctx: m.Infra.DetectorContext,
    ) -> t.SequenceOf[m.Infra.FLEXTCompletenessViolation]:
        """Detect missing FLEXT bases: expected - declared = violations."""
        ...
        if family is None:
            return []
        if not FlextInfraFLEXTCompletenessDetector._is_src_root_facade(ctx):
            return []
        ...
        declared = FlextInfraFLEXTCompletenessDetector._expand_declared_bases(
        ...
        return [
            m.Infra.FLEXTCompletenessViolation(
                file=str(file_path),
                line=line,
                family=family,
                facade_class=facade,
...
__all__: list[str] = ["FlextInfraFLEXTCompletenessDetector"]

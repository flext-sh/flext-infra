...
class FlextInfraRefactorMROImportRewriterFileOpsMixin:
    ...
    Composed into FlextInfraRefactorMROImportRewriter via inheritance; the
    classmethods resolve their intra-cluster siblings through ``cls`` MRO.
    """
    ...
    def _protected_source_write(
        *, workspace_root: Path, file_path: Path, updated_source: str
    ) -> t.Infra.EditResult:
    ...
    def _write_pending_sources(
        cls, *, workspace_root: Path, pending_sources: t.MappingKV[Path, str]

"""Automated migration of loose constants/typings into FLEXT facade classes."""
...
from flext_infra.refactor._migrate_flext_report import (
    FlextInfraRefactorMigrateFlextReportMixin,
)
from flext_infra.refactor.flext_import_rewriter import (
    FlextInfraRefactorFLEXTImportRewriter,
)
from flext_infra.refactor.flext_migration_validator import (
...
class FlextInfraRefactorMigrateToClassFLEXT(FlextInfraRefactorMigrateFlextReportMixin):
    ...
    def run(
        ...
        *,
        ...
        scan_results, files_scanned = (
            FlextInfraUtilitiesRefactorFlextScan.scan_workspace(
        ...
        if apply:
            ...
            checkpoint_outcome = safety_manager.create_pre_transformation_checkpoint(
                ...
                label="flext-infra-refactor-migrate-to-class-flext",
            )
        ...
        if apply:
            remaining_violations, flext_failures = (
                FlextInfraRefactorFLEXTMigrationValidator.validate(
                    workspace_root=self._workspace_root,
                    target=normalized_target,
                    project_names=project_names,
                )
            ...
            if safety_manager is not None:
                if errors or flext_failures:
                    warnings.append(
                        "Partial failures left in place (fix-forward): "
                        f"{len(errors)} error(s), {flext_failures} flext failure(s); "
        ...
        return m.Infra.FLEXTMigrationReport(
    ...
    @classmethod
    def execute_command(
        cls, params: m.Infra.RefactorMigrateFlextInput
    ) -> p.Result[m.Infra.FLEXTMigrationReport]:
        """Execute FLEXT migration directly from the canonical refactor payload."""
        ...
        if report.errors:
            return r[m.Infra.FLEXTMigrationReport].fail("FLEXT migration had errors")
        return r[m.Infra.FLEXTMigrationReport].ok(report)

    @classmethod
    def run_as_hook(cls, path: Path, *, dry_run: bool) -> t.SequenceOf[m.Infra.Result]:
        """Execute FLEXT migration as a rope post-hook (implements p.Infra.RopePostHook)."""
        try:
    ...
    def _normalize_target(*, target: str) -> str:
        ...
        if value in c.Infra.FLEXT_TARGETS:
            return value
        msg = f"unsupported target: {target}"
        raise ValueError(msg)


__all__: list[str] = ["FlextInfraRefactorMigrateToClassFLEXT"]

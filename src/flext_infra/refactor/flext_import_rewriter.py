"""Workspace-wide FLEXT migration orchestration and reference rewriting."""
...
from flext_infra.refactor._flext_import_collect import (
    FlextInfraRefactorFLEXTImportRewriterFileOpsMixin,
)
from flext_infra.transformers.flext_symbol_propagator import (
    FlextInfraRefactorFLEXTSymbolPropagator,
)


class FlextInfraRefactorFLEXTImportRewriter(
    FlextInfraRefactorFLEXTImportRewriterFileOpsMixin
):
    """Rewrite imports/references after FLEXT symbol absorption into facade classes."""
    ...
    def migrate_workspace(
        ...
        *,
        workspace_root: Path,
        scan_results: t.SequenceOf[m.Infra.FLEXTScanReport],
        apply: bool,
        ...
        errors: list[str] = []
        migrations: list[m.Infra.FLEXTFileMigration] = []
    ...
    def rewrite_workspace(
        ...
        *,
        ...
        project_names: t.StrSequence | None = None,
        gates: t.StrSequence | None = None,
    ) -> tuple[t.SequenceOf[m.Infra.FLEXTRewriteResult], t.StrSequence]:
    ...
    def _rewrite_files(
        ...
        """Rewrite files."""
        rewrites: list[m.Infra.FLEXTRewriteResult] = []
        errors: list[str] = []
        for file_path in sorted(request.file_moves):
            ...
            if source is None:
                ...
                if read.failure:
                    ...
                    continue
                source = read.value
            transformer = FlextInfraRefactorFLEXTSymbolPropagator(
            ...
            if request.apply:
                ...
                if not ok:
                    ...
                    continue
            rewrites.append(
                m.Infra.FLEXTRewriteResult(
                    file=str(file_path), replacements=len(changes)
                )
            )
        return (tuple(rewrites), tuple(errors))


__all__: list[str] = ["FlextInfraRefactorFLEXTImportRewriter"]

"""FLEXT-migration reporting — extracted concern of FlextInfraRefactorMigrateToClassFLEXT."""
...
class FlextInfraRefactorMigrateFlextReportMixin:
    """Render + convert the FLEXT migration report into text / rope Results.

    Composed into FlextInfraRefactorMigrateToClassFLEXT via inheritance; both
    ...
    @staticmethod
    def render_text(report: m.Infra.FLEXTMigrationReport) -> str:
        ...
        lines = [
            ...
            f"FLEXT failures: {report.flext_failures}",
    ...
    def _report_to_results(
        *, report: m.Infra.FLEXTMigrationReport, dry_run: bool
    ) -> t.SequenceOf[m.Infra.Result]:
        """Convert FLEXT migration report into rope-compatible Result sequence."""
        ...
        for migration in report.migrations:
            ...
            changes.extend([
                ("planned FLEXT migration" if dry_run else "applied FLEXT migration")
                + f": {symbol}"
        ...
        for rewrite in report.rewrites:
            ...
            changes.append(
                f"{action} {rewrite.replacements} consumer references after FLEXT migration"
            )
        return [
            m.Infra.Result(
                file_path=file_path,
...
__all__: list[str] = ["FlextInfraRefactorMigrateFlextReportMixin"]

...
class FlextInfraCodegenQualityGate(s[bool]):
    """Report constants migration outcomes from the canonical census."""
    ...
    def build_report(self) -> p.Result[t.JsonMapping]:
        ...
        after_metrics = self.after_metrics(census_report=census_report)
        ...
        report_data = {
            ...
            "projects": [
                ...
                for project in sorted(
                    census_report.projects, key=lambda entry: entry.project
                )
            ],
        }
    ...
    def after_metrics(
        *, census_report: m.Infra.Census.WorkspaceReport
        ...
        """Build the measured constants-census summary."""
    ...
    def build_checks(
        *, after_metrics: t.MappingKV[str, t.Infra.InfraValue]
        ...
        """Build checks backed exclusively by measured census metrics."""

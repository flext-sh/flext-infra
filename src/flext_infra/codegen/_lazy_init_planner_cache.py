...
class FlextInfraCodegenLazyInitPlannerCacheMixin:
    ...
    def _package_init_exports(self, package_name: str) -> frozenset[str]:
        ...
        # A generated __init__ is an output, not a declaration owner, wherever
        # it lives: reading an installed manifest made the plan depend on the
        # parent revision the environment happens to hold.
    ...
    def _source_export_names_for_package(self, package_name: str) -> frozenset[str]:
        """Read parent declarations from their authoritative sibling modules.

        Generated initializers are outputs of this plan, not declaration owners.
        Reuse the same source inventory that elects inherited aliases (indexed
        or environment-declared, R32); never retain a generated manifest.
        """

...
class FlextInfraCodegenLazyInitGenerationFilePlanMixin:
    ...
    def _verify_snapshots(
        ...
        analysis. If content differs, the snapshot is updated to the current
        state to maintain pipeline idempotence — the drift is logged but does
        not block the pipeline, as the planner operates on the current state.
        """
        for path, expected in snapshots.items():
            ...
            if expected_content is not None and current_content is not None:
                ...
                if expected_norm != current_norm:
                    ...
                    # The planner operates on current state, so this is safe

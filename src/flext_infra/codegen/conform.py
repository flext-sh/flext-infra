...
class FlextInfraCodegenConform(s[m.Infra.CodegenResult]):
    ...
    def _plan_ast_grep_surfaces(
        ...
        *,
        ...
        """Project package-owned ast-grep rules and fixtures into one repository.

        Why (mro-ga9q): the rendered ``sgconfig.yml`` points every governed
        repository at ``ast-grep-rules/`` + ``ast-grep-rule-tests/``, but the
        rule declarations themselves had no projection owner, so no repository
        could satisfy the contract. The SSOT is the codemod package
        (``flext_infra/codemod/rules`` + ``flext_infra/codemod/tests``);
        conform projects it under a generated marker so ``make gen``
        distributes rules project- and subproject-wide and drift fails closed.
        Stale projections (marker present, SSOT gone) are pruned; hand-written
        rules without the marker are never touched.
        """
        codemod_root = self._package_root() / "codemod"
        surfaces = [(codemod_root / "rules", Path(codegen.sgconfig.rule_dirs[0]))]
        if codegen.sgconfig.test_dirs:
            surfaces.append((
                codemod_root / "tests",
                Path(codegen.sgconfig.test_dirs[0]),
            ))
        planned: list[m.Infra.CodegenFilePlan] = []
        projected: set[Path] = set()
        for source_dir, destination_dir in surfaces:
            if not source_dir.is_dir():
                return r[t.SequenceOf[m.Infra.CodegenFilePlan]].fail(
                    f"codemod ast-grep SSOT directory is missing: {source_dir}"
                )
            for source in sorted(source_dir.rglob("*.yml")):
                relative = destination_dir / source.relative_to(source_dir)
                projected.add(relative)
                if (
                    contract.destinations is not None
                    and relative.as_posix() not in contract.destinations
                ):
                    continue
                read = u.Cli.files_read_text(source)
                if read.failure:
                    return r[t.SequenceOf[m.Infra.CodegenFilePlan]].fail(
                        read.error or f"codemod ast-grep SSOT read failed: {source}"
                    )
                marker = (
                    "# @generated by flext_infra codegen from "
                    "src/flext_infra/codemod/"
                    f"{source.relative_to(codemod_root).as_posix()} -- DO NOT EDIT.\n"
                    "# Run `make gen` to regenerate. Change the codemod SSOT,"
                    " never this projection.\n"
                )
                file_plan = self._file_plan(
                    root, relative.as_posix(), f"{marker}{read.value}"
                )
                if file_plan.failure:
                    return r[t.SequenceOf[m.Infra.CodegenFilePlan]].fail(
                        file_plan.error
                        or f"ast-grep projection planning failed: {relative}"
                    )
                planned.append(file_plan.value)
        if contract.destinations is not None:
            # Partial surfaces plan only their selected destinations and never
            # prune projections owned by the full surface.
            return r[t.SequenceOf[m.Infra.CodegenFilePlan]].ok(tuple(planned))
        for _source_dir, destination_dir in surfaces:
            target_dir = root / destination_dir
            if not target_dir.is_dir():
                continue
            for existing in sorted(target_dir.rglob("*.yml")):
                if existing.relative_to(root) in projected:
                    continue
                current = u.Cli.files_read_text(existing)
                if current.failure:
                    return r[t.SequenceOf[m.Infra.CodegenFilePlan]].fail(
                        current.error or f"ast-grep projection read failed: {existing}"
                    )
                if not current.value.startswith("# @generated by flext_infra codegen"):
                    continue
                planned.append(
                    m.Infra.CodegenFilePlan(
                        path=existing,

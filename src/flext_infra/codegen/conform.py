...
class FlextInfraCodegenConform(s[m.Infra.CodegenResult]):
    ...
    def _uv_install_projects(
        repository_root: Path, workspace: m.Infra.WorkspaceSpec
        ...
        if (workspace_root / ".git").exists():
            ...
            if superproject_path:
                ...
                declared = FlextInfraWorkspaceDetector.load_workspace_spec(
                    workspace_root
                )
                if declared.failure:
    ...
    def retired_projection_plans(
        ...
        for filename in codegen.toolchain.retired_dependency_artifacts:
            ...
            if path.is_symlink() or not path.is_file():
                raise ValueError(
                    f"Refusing non-file retired dependency artifact: {path}"
                )

...
class FlextInfraCodegenConform(s[m.Infra.CodegenResult]):
    ...
    def _plan_existing_templates(
        ...
        *,
        ...
        """Render existing-tree artifacts from their typed canonical owners."""
        if repository.profile is None:
            return r[t.SequenceOf[m.Infra.CodegenFilePlan]].fail(
                f"active repository has no Make profile: {repository.name}"
            )
        ...
        for managed in codegen.managed_files:
            ...
            if managed.policy == "create-only":
                ...
                continue
            context_result = self._existing_artifact_context(
                dist=dist,
                repository=repository,
                workspace=workspace,
                codegen=codegen,
    ...
    @staticmethod
    def _existing_artifact_context(
        *,
        dist: str,
        repository: m.Infra.RepositoryRef,
        workspace: m.Infra.WorkspaceSpec,

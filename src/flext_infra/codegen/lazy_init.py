...
class FlextInfraCodegenLazyInit(s[bool], FlextInfraCodegenLazyInitGenerationMixin):
    ...
    def generate_inits(self, *, check_only: bool = False) -> int:
        ...
        with FlextInfraRopeWorkspace.open_workspace(self.workspace_root) as rope:
            ...
            if target_package_dir is None:
                # Operator law 2026-08-16: lazy init generates EVERY Python
                # package dir (src, tests, scripts, examples, ...) with no
                # surface exclusions; wrapper roots are private plumbing but
                # their initializers are generated by the same default pass.
    ...
    def batch_lint_generated(self, generated_files: t.StrSequence) -> int:
        ...
        Returns the number of artifacts that failed the check.
        """
        targets = tuple(
            path

"""Bootstrap-safe project and target catalog for FLEXT scans."""
...
class FlextInfraUtilitiesFlextScanCatalog:
    """Resolve scan targets and Python files without importing flext_infra facade."""

    @classmethod
    def target_specs(
        cls, target: str
    ) -> tuple[FlextInfraModelsFlextScan.FLEXTTargetSpec, ...]:
        """Return target specs for a normalized FLEXT scan target."""
        ...
        specs = FlextInfraConstantsRefactor.FLEXT_TARGET_SPECS
        return (
    ...
    def target_supported(target: str) -> bool:
        """Return whether ``target`` is a supported FLEXT scan target."""
        return target in FlextInfraConstantsRefactor.FLEXT_TARGETS
    ...
    @staticmethod
    def matches_target(
        file_path: Path, spec: FlextInfraModelsFlextScan.FLEXTTargetSpec
    ) -> bool:
...
__all__: list[str] = ["FlextInfraUtilitiesFlextScanCatalog"]

"""Rope-driven migration of module constants into FLEXT constants facades.

Centralizes the FLEXT migration logic into the rope utility chain, removing
...
class FlextInfraUtilitiesRopeFlextTransform:
    ...
    def migrate_file(
        *, scan_result: m.Infra.FLEXTScanReport
        ...
        empty_migration = m.Infra.FLEXTFileMigration(
        ...
        for cand in candidates:
            ...
            if cand.symbol != target:
                block_lines = (
                    FlextInfraUtilitiesRopeFlextTransform._rename_symbol_in_block(
                        block_lines=block_lines, symbol=cand.symbol, target=target
                    )
                )

            moved_code = (
                FlextInfraUtilitiesRopeFlextTransform._indent_block(block_lines)
                + moved_code
            )
        ...
        lines = FlextInfraUtilitiesRopeFlextTransform._drop_redundant_class_aliases(
            lines=lines, class_name=class_name, symbol_map=symbol_map
        )
        new_source = "\n".join(lines) + "\n"
        new_source = FlextInfraUtilitiesRopeFlextTransform._qualify_local_references(
            source=new_source, facade_alias=facade_alias, symbol_map=symbol_map
        )

        migration = m.Infra.FLEXTFileMigration(
            file=scan_result.file,
            module=scan_result.module,
    ...
    def _qualify_local_references(
        *, source: str, facade_alias: str, symbol_map: t.StrMapping
        ...
        for symbol, target_path in symbol_map.items():
            ...
            bare_pattern = c.Infra.compile_flext_bare_qualify(symbol)
            annotation_pattern = c.Infra.compile_flext_prefixed_annotation(":", symbol)
            return_pattern = c.Infra.compile_flext_prefixed_annotation("->", symbol)
...
__all__: list[str] = ["FlextInfraUtilitiesRopeFlextTransform"]

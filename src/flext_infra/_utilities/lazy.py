...
class FlextInfraUtilitiesCodegenLazyScanning(FlextInfraUtilitiesCodegenLazyMerging):
    ...
    def discover_project_aliases(project_root: Path) -> t.StrMapping:
        """Discover all single-char aliases declared by project facade modules."""
        ...
        for facade_name in (
            ...
            if not facade_path.is_file():
                continue
            try:
                content = facade_path.read_text(encoding=c.Infra.Encoding.DEFAULT)
            except OSError:
                continue
            match = re.search(
                r"^\s*([a-z])\s*=\s*[A-Z][a-zA-Z0-9_]+\s*$",
                content,
                re.MULTILINE,
            )
    ...
    def _should_publish_local_symbol(
        ...
        *,
        ...
        return callable_name not in c.Infra.TYPEVAR_CALLABLES or (
            bool(parts) and (parts[-1] == "typings" or "_typings" in parts)
        )
...
class FlextInfraUtilitiesCodegenLazyAliases:
    ...
    def __init__(self, workspace_root: Path | None = None) -> None:
        ...
        self._lazy_init = config.unwrap().lazy_init
    ...
    def resolve_aliases(
        ...
        *,
        ...
        if surface not in self._lazy_init.surface_prefixes:
            surface = "src"
        ...
        self._inherit_project_source_aliases(
            lazy_map,
            pkg_dir=pkg_dir,
            surface=surface,
        )

    def _inherit_project_source_aliases(
        self,
        lazy_map: t.Infra.MutableLazyImportMap,
        *,
        pkg_dir: Path,
        ...
        """Inherit allowed aliases from this project's primary src package."""
        if surface == "src":
            return
        project_root = FlextInfraUtilitiesDiscovery.discover_project_root_from_file(
            pkg_dir,
        ...
        if project_root is None:
            return
        src_package_name = FlextInfraUtilitiesDocsScope.package_name(project_root)
        if not src_package_name:
            return
        src_package_dir = (
            project_root
            / c.Infra.Paths.DEFAULT_SRC_DIR
            / Path(
                *src_package_name.split("."),
            )
        )
        if not (src_package_dir / c.Infra.Files.INIT_PY).is_file():
            return
        if src_package_dir.resolve() == pkg_dir.resolve():
            return
        for alias_name, target in self._collect_package_aliases(
            src_package_dir,
            surface=surface,
    ...
    def _collect_package_aliases(
        ...
        *,
        ...
        allowed_exports = tuple(self._lazy_init.inherited_exports.get(surface, ()))
        resolved_aliases = {
            export_name: target

...
class FlextInfraUtilitiesRopeAnalysisSource(FlextInfraUtilitiesRopeAnalysisScope):
    ...
    def mapping_entries_refs(
        ...
        function_name = FlextInfraUtilitiesRopeAnalysisNodes.name_of(func) if func is not None and hasattr(func, "_fields") else ""
        args = getattr(node, "args", ()) or ()
        if function_name in {"MappingProxyType", "build_lazy_import_map"} and args:
            first_arg = args[0]
            if hasattr(first_arg, "_fields"):
                return FlextInfraUtilitiesRopeAnalysisSource.mapping_entries_refs(first_arg)
    ...
    def _dict_entries_refs(
        ...
        for key_node, value_node in zip(keys, values, strict=False):
            ...
            if not isinstance(key_value, str):
                continue
            if hasattr(value_node, "_fields"):
                value_strings = FlextInfraUtilitiesRopeAnalysisSource.literal_string_sequence(
                    value_node
                )
            else:
                value_strings = ()

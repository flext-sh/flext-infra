...
class FlextInfraConstantsBase(
    ...
    "Code-review-graph database directory environment key."
    ...
    CODE_REVIEW_GRAPH_INFO_LINE_PATTERNS: Final[tuple[str, ...]] = (
        ...
        r"INFO: Python import resolution: \{'files_indexed': \d+, 'imports_updated': \d+, 'imports_resolved': \d+, 'imports_ambiguous': \d+\}",
        r"INFO: ReScript cross-module resolution: \{'files_indexed': \d+, 'calls_resolved': \d+, 'imports_resolved': \d+\}",
        r"INFO: Spring DI resolver: resolved \d+ CALLS edges in \d+ Java files",
        r"INFO: Spring event resolver: indexed \d+ events and emitted \d+ CALLS edges",
        r"INFO: Temporal resolver: resolved \d+ CALLS edges in \d+ Java files",
        r"INFO: Terraform/HCL module resolution: \{'files_indexed': \d+, 'references_resolved': \d+, 'imports_resolved': \d+\}",

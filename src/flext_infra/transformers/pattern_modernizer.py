...
class FlextInfraRefactorPatternModernizer(FlextInfraRopeTransformer):
    ...
    _description = "fix common FLEXT anti-patterns (print, pdb, bare except, open encoding)"
    ...
    def apply_to_source(self, source: str) -> t.Infra.TransformResult:
        ...
        visitor = _PatternVisitor(source, module_has_logger=module_has_logger)
        visitor.visit(tree)
    ...
    @staticmethod
    def _inject_logger(source: str) -> str:
        """Inject ``logger = u.fetch_logger(__name__)`` after imports/docstring."""
        lines = source.splitlines(keepends=True)
        insert_idx = _find_insertion_after_header(lines)
        lines.insert(insert_idx, "logger = u.fetch_logger(__name__)\n")
        return "".join(lines)
    ...
    def _ensure_u_import(source: str) -> str:
        ...
        lines = source.splitlines(keepends=True)
        insert_idx = _find_insertion_after_header(lines)
        lines.insert(insert_idx, "from flext_core import u\n")
        return "".join(lines)


_DOCSTRING_MIN_ONE_LINER_LEN: int = 6
...
class _Rewrite:
    ...
    def __lt__(self, other: object) -> bool:
        if not isinstance(other, _Rewrite):
...
class _PatternVisitor(ast.NodeVisitor):
    ...
    def visit_Expr(self, node: ast.Expr) -> None:
        ...
        if isinstance(value, ast.Call) and isinstance(value.func, ast.Name):
            if value.func.id == "print":
                ...
                self.rewrites.append(_Rewrite(start, end, new_call))
                self.changes.append("Replaced print() with logger.info()")
                return
            if value.func.id == "breakpoint":
                start = self._node_offset(node, start=True)
                end = self._node_offset(node, start=False)
                self.rewrites.append(_Rewrite(start, end, "pass"))
                self.changes.append("Replaced breakpoint() with pass")
                return
    ...
    def visit_ExceptHandler(self, node: ast.ExceptHandler) -> None:
        ...
        if node.type is None:
            ...
            if stripped.startswith("except:"):
                ...
                self.rewrites.append(_Rewrite(start, end, new_line))
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if (
            ...
            if call_text.endswith(")"):
                ...
                self.rewrites.append(_Rewrite(start, end, new_call))
                self.changes.append('Added encoding="utf-8" to open()')
        self.generic_visit(node)
    ...
    def visit_Import(self, node: ast.Import) -> None:
        ...
        if any(alias.name == "pdb" for alias in node.names):
            ...
            self.rewrites.append(_Rewrite(start, end, "pass"))
            self.changes.append("Removed import pdb")
        self.generic_visit(node)
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        ...
        if node.module == "pdb":
            ...
            self.rewrites.append(_Rewrite(start, end, "pass"))
            self.changes.append("Removed from pdb import")
        self.generic_visit(node)


def _find_insertion_after_header(lines: list[str]) -> int:
    """Return line index after module docstring and imports."""
    idx = 0
    in_docstring = False
    docstring_quote = ""
    ...
    for quote in ('"""', "'''"):
        if first.startswith(quote):
            ...
            in_docstring = True
            idx = 1
            if first[3:].find(quote) != -1 or (
                len(first) >= _DOCSTRING_MIN_ONE_LINER_LEN and first.endswith(quote)
            ):
                in_docstring = False
            break

    while idx < len(lines) and in_docstring:
        if docstring_quote in lines[idx]:
            in_docstring = False
        idx += 1

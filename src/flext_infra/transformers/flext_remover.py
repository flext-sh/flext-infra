"""FLEXT redeclaration remover transformer — rope-based implementation."""
...
class FlextInfraRefactorFLEXTRemover(FlextInfraRopeTransformer):
    ...
    def transform(
        ...
        """Apply FLEXT redeclaration removal. Returns (new_source, changes)."""
    ...
    def _strip_parent_base(
        ...
        *,
        ...
        if count > 0 and new_source != source:
            self._record_change(f"Fixed FLEXT redeclaration: {nested_class}")
            return new_source
        return source


__all__: list[str] = ["FlextInfraRefactorFLEXTRemover"]

...
def render_template(template_path: Path) -> r[str]:
    ...
    header_template = u.Infra.GENERATED_SHELL_HEADER
...
def sync_workspace(
    ...
    *,
    ...
    source_result = resolve_source_workflow(workspace_root, source_workflow)
    if source_result.is_failure:
        return r[list[SyncOperation]].fail(
            source_result.error or "source resolution failed",
        )
    template_result = render_template(source_result.value)
    ...
    if report_path is not None:
        write_report(report_path, apply=apply, operations=all_operations)

...
def run_dispatch(
    ...
    *,
    ...
    if not args or args[0] in {"help", "--help", "-h"}:
        requested = (current_settings().Infra.dispatch_what or "").strip()
...
def dispatch(registry: Registry, requested_verb: str) -> int:
    ...
    requested_what = (current_settings().Infra.dispatch_what or "").strip()
...
def require_dispatched(path: Path) -> None:
    ...
    if (
        current_settings().Infra.cosmos_command_dispatched == "Y"
        and current_settings().Infra.cosmos_command_path == expected
    ):
        return
    sys.stderr.write(
